...
func TestSplitPreservesTheAggregate(t *testing.T) {
	...
	// this replaces.
	// 119,489,000, not the 117,648,000 this branch was written against: the
	// GovDAO T1 line itself has moved seven times on main since then. Skipping
	...
	// founding validator set's gas float takes 6,000 out of §122, and the two
	// chain-service floats take 10,000 out of §120.
	// Expect this to drop to 119,349,000 when the remaining 14 qualifying
	// authors get keys.
	assert.Equal(t, 119489000,
	...
	assert.Equal(t, 119489000+TOTAL_AIRDROP_GOVDAO_FOUNDERS+TOTAL_PREMINE_NON_AIRDROP+
